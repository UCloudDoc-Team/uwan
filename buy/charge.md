...
| **计费模式** |**计费方式**| **计费单位**                                                 | **适用场景**                     |
| ------------ | ------------ | ------------------------------------------------------------ | -------------------------------- |
| 固定带宽     | 预付费       | 元/月，最少购买时长为1个月；                                 | 适用于带宽稳定的场景，峰值为出入向总量    |
| 流量计费       | 后付费       | 元/月，实例需预付费，最少购买时长为1个月；<br />元/GB，流量费用根据每天实际使用的出入向总流量计费； |适用于爆发业务的场景 |
...
>**带宽计费说明**
>
>* **带宽上限限制**：带宽上限限制的是出入向总带宽（入向+出向之和不超过带宽上限）。
...
**计费示例**
    ...
    例如：客户A，于8月5日上午10点30分0秒，以带宽计费模式购买了配置为100M的UWAN，则客户A本月账单为：
    ...
    本月实际账单为：（90 + 100 * 110）* 0.8569= 9503.02 元
...
>**流量计费说明**
>
>* **带宽上限限制**：带宽上限限制的是出入向总流量（入向+出向之和不超过带宽上限）。
>* **流量计费规则**：出入向流量均计入费用，按双向流量总和计费。
...
**计费算法**
    ...
    流量费= 日流量总量（入向+出向） * 流量单价 ；    

**计费示例**

    例如：客户B，于8月5日上午10点30分0秒，以流量计费模式购买了配置为100M的UWAN，则客户B本月账单为：
    ...
    月流量用量为：10000 GB（入向+出向总和）；
            
    当月有效时间系数为：26天13小时30分0秒/31天=（2246400+46800+1800）/2678400= 0.8569 ；
    
    （8月份总共31天，用户8月5日上午10点30分0秒购买的，客户的实际使用时长为8月5日这天从10点30分0秒开始计费至24点+8月6日至8月31日全天，总共26天13小时30分0秒，总共2295000秒）
    
    本月实例账单为：90 * 0.8569 + 0.9 * 10000  = 9077.12 元